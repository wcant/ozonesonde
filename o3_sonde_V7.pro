...
WHILE NOT EOF(lun) DO BEGIN
  ...
  day[k]=FIX(STRMID(date[k], 0,2))
  month[k]=FIX(STRMID(date[k],3,2))
  year[k]=FIX(STRMID(date[k],6,4))
...
max_alt=MAX(WHERE(alt EQ MAX(alt)))
temp=temp[0:max_alt]
hum=hum[0:max_alt]
...
ouf1=file+'1.png'          ;whole plot
ouf2=file+'2.png'          ;tropospheric plot
ouf3=file+'3.png'          ;Potential Temperature Plot   -Wesley Cantrell 7/30/2009
ouf4=file+'4.png'          ;0-500km mixing ratio         -Wesley Cantrell 10/30/2010
...
xyouts,0.5,0.08,'Surface T='+STRCOMPRESS(STRING(temp[launch_index],FORMAT='(f4.1)'))+'(!UO!NC)'+$
       '; Surface O3='+STRCOMPRESS(STRING(o3mr_imet[launch_index]*1000.,FORMAT='(f4.1)'))+'(ppbv)',/normal,ALIGNMENT=0.5
xyouts,15.2,37.3,'O3'
...
oplot,(temp+80.)/6.,alt,linestyle=3,color=40
;humdity,remove missing value--9999
axis,0,44,xaxis=1,XRANGE =!x.CRANGE*5,xstyle=1,xtitle='Relative Humidity(%)'
  oplot,hum/5.,alt,linestyle=1,color=10

;-----------------------------
;Plot mixing ratio
plot,o3mr_imet*1000,alt,clip=[0,0,200,15],$
...
oplot,o3mr_imet*10,alt,linestyle=1,clip=[0,15,200,40]
...
oplot,(temp+80.)/12.,alt,linestyle=3,color=40
;humdity
axis,0,19.8,xaxis=1,XRANGE =!x.CRANGE*10,xstyle=1,xtitle='Relative Humidity(%)'
  oplot,hum/10.,alt,linestyle=1,color=10
...
oplot,(ptemp-250.)/7.5,alt,linestyle=3,color=5
...
plot,o3mr_imet*1000,alt,clip=[0,0,200,20],$
     xtitle='O3 Mixing Ratio(ppbv)',$
     yticklen=0.5,ygridstyle=1,$
...
oplot,o3mr_imet*10,alt,linestyle=1,clip=[0,15,200,20]
xyouts,0.5,0.92,'Huntsville Ozonesonde Data',charsize=3.,/normal,ALIGNMENT=0.5
...
plot, o3mr_imet*1000, alt, yrange=[0.196, 0.5]  $
...
xyouts,0.5,0.4,'Surface T='+STRCOMPRESS(STRING(temp[launch_index],FORMAT='(f4.1)'))+'(!UO!NC)'+$
       '; Surface O3='+STRCOMPRESS(STRING(o3mr_imet[launch_index]*1000.,FORMAT='(f4.1)'))+'(ppbv)',/normal,ALIGNMENT=0.5
